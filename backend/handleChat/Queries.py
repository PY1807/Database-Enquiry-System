...
collection = db['Client']  # Replace 'Client' 
...
def handlequery(query):
    ...
    if isinstance(query_to_execute, list):
        print("Detected an aggregation query.")
      
        try:

            query_result = collection.aggregate(query_to_execute)
            query_result = list(query_result)  # 
            print(json_util.dumps(query_result, indent=4))
    ...
    try:
        ...
        if isinstance(query_result, type(collection.find())):  
            query_result = list(query_result)
